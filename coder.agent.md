...
3. **After all phases complete:**
   ...
   - Offer to commit and push changes (see [Committing Changes](#committing-changes-include-related-documents) — always include plan and research documents)
...
### Committing Changes (Include Related Documents)

🚨 **CRITICAL: When committing implementation code, ALWAYS include the related research and plan documents in the same commit.** These documents are part of the deliverable and must travel with the code they describe.

**What to include in every commit:**

1. **The implementation plan document** — The plan file (e.g., `/docs/plans/NNN-feature.md`) that drove the implementation
2. **Related research documents** — Any research files referenced in the plan's metadata or linked in its content (typically in `/docs/research/`)
3. **Related vision documents** — Any vision files that originated the work (typically in `/docs/vision/`)
4. **Data contract files** — Any contracts created or updated during implementation (in `/docs/data-contracts/`)
5. **All implementation code** — Source files created or modified during the implementation

**How to identify related documents:**

- Check the plan document's header/metadata for references to research or vision documents
- Look for explicit links like `Research: /docs/research/NNN-topic.md` or `Vision: /docs/vision/NNN-feature.md`
- If the plan path is `/docs/plans/NNN-feature.md`, check for matching research at `/docs/research/NNN-*.md`
- Include any document in `/docs/` that was created or modified as part of this implementation workflow
...
# Stage the plan document (ALWAYS)
git add docs/plans/NNN-feature.md

# Stage related research documents
git add docs/research/NNN-*.md

# Stage related vision documents (if any)
git add docs/vision/NNN-*.md

# Stage data contracts (if created/modified)
git add docs/data-contracts/*.md
```

**Commit message format:**

```
feat: [brief description of implementation]

Implemented per plan docs/plans/NNN-feature.md
Includes: plan, research, and implementation code
```

**Why this matters:**
- Research and plan documents provide context for code review
- Keeping documents with code ensures traceability
- Future developers can understand WHY code was written by reading the co-located plan
- Orphaned documents without their code (or code without its documents) lose their value
...
7. **Offer to commit and push** — Ask the user: "Would you like me to commit and push all changes to the repository?" (When committing, follow [Committing Changes](#committing-changes-include-related-documents) — always include the plan and related research/vision documents in the commit)
...
2. **Commit changes** — Stage and commit all implementation changes plus related plan/research documents (see [Committing Changes](#committing-changes-include-related-documents))
