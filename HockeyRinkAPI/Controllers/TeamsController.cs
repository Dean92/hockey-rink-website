    ...
    public async Task<IActionResult> GetDraftPlayers(int sessionId)
    {
        try
        {
            ...
            var players = await _dbContext.SessionRegistrations
                ...
                .OrderByDescending(sr => sr.User.Rating ?? 0) // Sort by user rating (highest first), nulls at bottom
                .ThenBy(sr => sr.Name)
                .Select(sr => new
                {
                    sr.Id,
                    sr.Name,
                    sr.Position,
                    Rating = sr.User.Rating, // Use ApplicationUser rating instead of SessionRegistration
                    sr.Email,
